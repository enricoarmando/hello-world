...
switch ($act) {
	case 'simpan_trans' :
		...
		// query detail
		$sql = $db->update('tsodtl', array('tutup' => 0), array('kodeso' => '', 'kodebarang' => '', 'kodebarangdtl' => ''), $tr, true);
		$pr  = $db->prepare($sql, $tr);
		foreach ($a_detail as $item) {
			$data_values = array (
				$item->tutup, $item->kodeso, $item->kodebarang, $item->kodebarangdtl
			);
	...
	case 'tampil_data' :
		...
		$sql = "select A.TGLTRANS, A.KODESO, A.KODECUSTOMER, A.NAMACUSTOMER, A.TOTAL, A.PPNRP, A.GRANDTOTAL, A.CATATAN, A.USERENTRY,
				...
				group by A.TGLTRANS, A.KODESO, A.KODECUSTOMER, A.NAMACUSTOMER, A.TOTAL, A.PPNRP, A.GRANDTOTAL, A.CATATAN, A.USERENTRY, A.STATUS, A.TGLINPUT
				$temp_status";
		$query = $db->query($sql);
		$rows = array();
