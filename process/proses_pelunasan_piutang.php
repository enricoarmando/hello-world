...
switch ($act) {
	case 'simpan_trans' :
		...
		if ($mode=='tambah') {
			...
			$urutan = get_new_urutan('pelunasanpiutang', 'kodepelunasan', array($temp_kode, substr($tgltrans, 2, 2)));

			$kodetrans = $temp_kode.$urutan.'/'.substr($tgltrans, 5, 2).'/'.substr($tgltrans, 2, 2);*/

			$temp_kode = 'PP/'.$_SESSION['KODELOKASI'].'/';

			$urutan = get_max_urutan('pelunasanpiutang', 'kodepelunasan', $temp_kode, 4);
		...
		$data_values = array(
			$kodetrans, $_POST['KODELOKASI'], $_POST['KODEINSTANSI'], '', $kodekas,
			$kodememo, $tgltrans, date("Y.m.d"), date("H:i:s"), '',
			$nobg, $total_debet, $namabank, $tglcairbg, $statusbg,
			$_SESSION['user'], $total_debet, '', $ket, 'S',
			0
		);
		$exe = $db->insert('pelunasanpiutang', $data_values, $tr);
		...
		$sql = $db->insert('pelunasanpiutangdtl', 7, $tr, true);
		$pr  = $db->prepare($sql, $tr);
		foreach ($data as $item) {
			$data_values = array(
				$kodetrans, $item->kodecustomer, $item->kodetrans, $item->pelunasan, $item->pelunasan,
				$item->pelunasan, ''
			);
		...
		$tr  = $db->start_trans();
		$query = $db->update('pelunasanpiutang', array('status' => 'S'), array('kodepelunasan' => $kodetrans), $tr);
		$db->commit($tr);
	...
	case 'load_data' :
		...
		$sql = "select b.kodetrans, d.tgltrans, d.jtrans, d.kodecustomer, c.namacustomer, d.tgljatuhtempo, b.amount, d.sisa
				...
				left join mcustomer c on d.kodecustomer = c.kodecustomer
	...
	case 'batal_trans' :
		$kodetrans = $_POST['kodetrans'];
		$status    = get_status('pelunasanpiutang', 'kodepelunasan', $kodetrans);

		if ($status=='P') die(json_encode(array('errorMsg' => 'Transaksi Tidak Dapat Dibatalkan')));

		$tr  = $db->start_trans();
		$query = $db->update('pelunasanpiutang', array('status' => 'D'), array('kodepelunasan' => $kodetrans), $tr);
		$db->commit($tr);
	...
	case 'tampil_data' :
		...
		$sql = "select a.kodetrans, a.nofaktur, a.jtrans, a.tgltrans, a.kodecustomer, c.namacustomer, a.tgljatuhtempo, a.grandtotal, a.sisa
				from kartupiutang a
				inner join mcustomer c on a.kodecustomer = c.kodecustomer
				where a.sisa!=0 $temp_sql and
					  a.kodeinstansi='".$_POST['KODEINSTANSI']."' and
...
function insert_detail_kodeperkiraan($jtrans, $amount, $ket = '') {
	...
	if ($jtrans=='PIUTANG') {
		$jenis = 'JUAL-PIUTANG';
		$keterangan = 'ANGSURAN PIUTANG OLEH '.$ket;
