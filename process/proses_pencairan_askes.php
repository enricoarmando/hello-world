...
switch ($act) {
	case 'simpan_trans' :
		...
		if ($mode=='tambah') {
			$temp_kode = 'PA/'.$_SESSION['KODELOKASI'].'/';

			$urutan = get_max_urutan('pelunasanpiutangaskes', 'kodepelunasan', $temp_kode, 4);
		...
		$exe = $db->insert('pelunasanpiutangaskes', $data_values, $tr);
		...
		$sql = $db->insert('pelunasanpiutangaskesdtl', 4, $tr, true);
		...
		$tr  = $db->start_trans();
		$query = $db->update('pelunasanpiutangaskes', array('status' => 'S'), array('kodepelunasan' => $kodetrans), $tr);
		$db->commit($tr);

		$tr  = $db->start_trans();
		$pr  = $db->prepare('execute procedure tutup_kartu_piutang_askes(?)', $tr);
	...
	case 'load_data' :
		...
		$sql = "select b.kodetrans, d.tgltrans, b.amount, d.grandtotal, d.sisa, c.kodecustomer, c.namacustomer
				from pelunasanpiutangaskes a
				inner join pelunasanpiutangaskesdtl b on a.kodepelunasan = b.kodepelunasan
				inner join kartupiutangaskes d on d.kodetrans=b.kodetrans
	...
	case 'batal_trans' :
		$kodetrans = $_POST['kodetrans'];
		$status    = get_status('pelunasanpiutangaskes', 'kodepelunasan', $kodetrans);

		if ($status=='P') die(json_encode(array('errorMsg' => 'Transaksi Tidak Dapat Dibatalkan')));

		$tr  = $db->start_trans();
		$query = $db->update('pelunasanpiutangaskes', array('status' => 'D'), array('kodepelunasan' => $kodetrans), $tr);
		$db->commit($tr);
	...
	case 'tampil_data' :
		...
		$sql = "select a.kodetrans, a.tgltrans, a.grandtotal, a.sisa, c.kodecustomer, c.namacustomer
				from kartupiutangaskes a
