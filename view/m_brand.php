...
<div class="easyui-layout" style="width:100%;height:100%" fit="true">
	<div data-options="region:'north'" style="height:40px;padding:5px;">
		...
		<div style="float:right" hidden>
			...
			<div id="mm">
				<div data-options="name:'alasan'">Brand</div>				
			</div>
		</div>
	</div>
...
<div id="form_input" style="width:550px">
	...
	<table style="padding:5px">
		...
		<tr>
			<td align="right" id="label_form">Nama Brand</td>
...
function simpan() {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=brand&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
		...
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',
					{act:'delete', table:'brand', kode:row.KODE},function(msg){
					if (msg.success){						
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table() {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=brand',
		rowStyler: function(index,row){  
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODE',title:'Kode Brand',width:120, sortable:true,},
			{field:'NAMA',title:'Nama Brand',width:200, sortable:true,},
		]],
		columns:[[
