...
<div id="form_input" style="width:500px">
	<input type="hidden" name="act">
	<table style="padding:5px">
		<tr>
			<td align="right" id="label_form">Kode Kategori Supplier</td>
			<td><input name="KODEKATEGORISUPPLIER" style="width:100px" class="label_input" required="true" validType='length[0,10]'><label id="label_form"><input type="checkbox" id="STATUS" name="STATUS" value="1"> Aktif</label></td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama Kategori Supplier</td>
			<td><input name="NAMAKATEGORISUPPLIER" style="width:300px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
	</table>
</div>
<div id="dlg-buttons">
...
function simpan() {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=kategori_supplier&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
...
function hapus() {
	...
	if (row){
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',{act:'delete', table:'kategori_supplier', kodekategorisupplier:row.KODEKATEGORISUPPLIER},function(msg){
					if (msg.success){						
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table() {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=kategori_supplier',
		rowStyler: function(index,row){  
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODEKATEGORISUPPLIER',title:'Kode Kategori Supplier',width:120, sortable:true,},
			{field:'NAMAKATEGORISUPPLIER',title:'Nama Kategori Supplier',width:200, sortable:true,},
		]],
		columns:[[
