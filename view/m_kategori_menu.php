...
<div id="form_input" style="width:550px">
	...
	<table style="padding:5px">
		<tr>
			...
			<td>
				<input name="KODEKATEGORIMENU" style="width:100px" class="label_input" required="true" validType='length[0,10]'>
				<label id="label_form"><input type="checkbox" id="STATUS" name="STATUS" value="1"> Aktif</label>
			</td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama Kategori Menu</td>
			<td><input name="NAMAKATEGORIMENU" style="width:200px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
		<tr>
...
function simpan() {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=kategori_menu&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
		...
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',
					{act:'delete', table:'kategori_menu', kodekategorimenu:row.KODEKATEGORIMENU},function(msg){
					if (msg.success){						
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table() {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=kategori_menu',
		rowStyler: function(index,row){  
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODEKATEGORIMENU',title:'Kode Kategori Menu',width:120, sortable:true,},
			{field:'NAMAKATEGORIMENU',title:'Nama Kategori Menu',width:200, sortable:true,},
		]],
		columns:[[
			...
			{field:'URUTAN',title:'Urutan',width:70, sortable:true,align:'right'},
