...
<div class="easyui-layout" style="width:100%;height:100%" fit="true">
	<div data-options="region:'north'" style="height:40px;padding:5px;">
		...
		<div style="float:right" hidden>
			...
			<div id="mm">
				<div data-options="name:'alasan'">Jenis Barang</div>				
			</div>
		</div>
	</div>
...
<div id="form_input" style="width:550px">
	...
	<table style="padding:5px">
		<tr>
			<td align="right" id="label_form">Kode Jenis Barang</td>
			<td>
				<input name="KODEJENISBARANG" style="width:100px" class="label_input" required="true" validType='length[0,10]'>
				<label id="label_form"><input type="checkbox" id="STATUS" name="STATUS" value="1"> Aktif</label>
			</td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama Jenis Barang</td>
			<td><input name="NAMAJENISBARANG" style="width:200px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
	</table>
</div>
<div id="dlg-buttons">
...
function simpan() {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=jenis_barang&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
		...
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',
					{act:'delete', table:'jenis_barang', kodejenisbarang:row.KODEJENISBARANG},function(msg){
					if (msg.success){						
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table() {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=jenis_barang',
		rowStyler: function(index,row){  
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODEJENISBARANG',title:'Kode Jenis Barang',width:120, sortable:true,},
			{field:'NAMAJENISBARANG',title:'Nama Jenis Barang',width:200, sortable:true,},
		]],
		columns:[[
