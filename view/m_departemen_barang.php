...
<div id="form_input" style="width:550px">
	...
	<table style="padding:5px">
		<tr>
			<td align="right" id="label_form">Kode Departemen Barang</td>
			<td><input name="KODEDEPARTEMENBARANG" style="width:100px" class="label_input" required="true" validType='length[0,10]'><label id="label_form"><input type="checkbox" id="STATUS" name="STATUS" value="1"> Aktif</label></td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama Departemen Barang</td>
			<td><input name="NAMADEPARTEMENBARANG" style="width:300px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
		<tr>
			<td align="right" id="label_form">PLU</td>
			<td><input name="PLU" style="width:300px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama Departemen Barang</td>
			<td><input name="KETERANGAN" style="width:300px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
	</table>
</div>
<div id="dlg-buttons">
...
function simpan() {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=departemen_barang&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
...
function hapus() {
	...
	if (row){
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',{act:'delete', table:'departemen_barang', kodedepartemenbarang:row.KODEDEPARTEMENBARANG},function(msg){
					if (msg.success){						
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table() {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=departemen_barang',
		rowStyler: function(index,row){  
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODEDEPARTEMENBARANG',title:'Kode Dept. Barang',width:120, sortable:true,},
			{field:'NAMADEPARTEMENBARANG',title:'Nama Departemen Barang',width:200, sortable:true,},
		]],
		columns:[[
			{field:'PLU',title:'PLU',width:100, sortable:true,},
			{field:'KETERANGAN',title:'Keterangan',width:200, sortable:true,},
