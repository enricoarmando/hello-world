...
<form method='post' target='_blank' action='data/report/lap_akun_kas.php' id="form_input" style="width:500px">
	<div style="padding:5px;border:1px solid #ddd;">
        <a id="btn_print" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-print', plain:true">Tampikan Data</a>
    ...
    <fieldset id="field">
        ...
        <table width="410">
            <tr>
                ...
                <td width=""><label id="label_laporan"><input type="checkbox" class="jtrans" name="cbJenis[]" value="KAS KELUAR" >Kas Keluar</label></td>
            </tr>
        </table>
    </fieldset>
    ...
    <fieldset id="field">
        ...
        <table width="466">
            <tr>
                <td width="43%"><label id="label_laporan"><input type="radio" name="rdTampil" value="Harian" checked="checked">Harian</label></td>
    ...
    <fieldset id="field">
        <legend id="label_laporan">Akun Kas/Bank</legend>
        <div style='overflow:auto; height:150px;'>
			<table id="table_kode_perkiraan"></table>
        </div>
    </fieldset>
	
	<input type="hidden" name="KeteranganHeader" id="KeteranganHeader" value="Laporan Kas">
...
$(document).ready(function(){
	...
	$(".jtrans").each(function(){
		if ($(this).val()=="KAS MASUK" || $(this).val()=="KAS KELUAR") {
	...
	$.ajax({
		type: 'POST',
		dataType: 'json',
		url: "config/combogrid.php?table=kode_perkiraan&jenis=kas",
		data: "", 
		cache: false, 
...
$("[name=rdTampil]").change(function(){
	if ($(this).val()=="Harian" || $(this).val()=="Detail") {
	    if ($(this).val()=="Harian") {
			$("#txt_tgl_ak").datebox('disable');
		} else {
		...
		$(".jtrans").each(function(){
			jtrans = $(this).val(); 
			if (jtrans=="KAS MASUK" || jtrans=="KAS KELUAR") {
				$(this).prop("checked", true);
			} else {
				$(this).prop({"checked":false, "disabled":true});
			}	
