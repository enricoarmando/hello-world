...
<div id="form_input" style="width:500px">
	...
	<table style="padding:5px">
		<tr>
			<td align="right" id="label_form">Kode Kategori Barang</td>
			<td><input name="KODEKATEGORIBARANG" style="width:100px" class="label_input" required="true" validType='length[0,10]'><label id="label_form"><input type="checkbox" id="STATUS" name="STATUS" value="1"> Aktif</label></td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama Kategori Barang</td>
			<td><input name="NAMAKATEGORIBARANG" style="width:300px" class="label_input" required="true" validType='length[0,50]'></td>
		</tr>
	</table>
</div>
<div id="dlg-buttons">
...
function simpan() {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=kategori_barang&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
...
function hapus() {
	...
	if (row){
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',{act:'delete', table:'kategori_barang', kodekategoribarang:row.KODEKATEGORIBARANG},function(msg){
					if (msg.success){						
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table() {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=kategori_barang',
		rowStyler: function(index,row){  
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODEKATEGORIBARANG',title:'Kode Kategori Barang',width:120, sortable:true,},
			{field:'NAMAKATEGORIBARANG',title:'Nama Kategori Barang',width:200, sortable:true,},
		]],
		columns:[[
