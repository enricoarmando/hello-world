...
<div class="easyui-layout" style="width:100%;height:100%" fit="true">
	<div data-options="region:'north'" style="height:40px;padding:5px;">
		...
		<div style="float:right" hidden>
			...
			<div id="mm">
				<div data-options="name:'namainstansi'">Nama</div>
				<div data-options="name:'kodeinstansi'">Kode</div>
			</div>
		</div>
	</div>
	<div data-options="region:'center',">
		<table id="table_data" idField="KODEINSTANSI"></table>
	</div>
</div>
...
<div id="form_input" style="width:600px">
	...
	<table style="padding:5px">
		<tr>
			...
			<td>
				<input name="KODEINSTANSI" style="width:130px" class="label_input" prompt="Auto Generate" readonly>
				<label id="label_form"><input type="checkbox" id="STATUS" name="STATUS" value="1"> Aktif</label>
			</td>
		</tr>
		<tr>
			<td align="right" id="label_form">Nama</td>
			<td><input name="NAMAINSTANSI" style="width:300px" class="label_input" required="true" validType='length[0,300]'></td>
		</tr>
		<tr>
		...
		<tr>
			<td align="right" id="label_form">Kecamatan</td>
			<td><input name="KECAMATAN" style="width:200px" class="label_input" validType='length[0,100]'></td>
		</tr>
		<tr>
...
function tambah () {
	...
	$('[name=JENIS]').filter(function(){
		return $(this).val()=='NON PENERBIT';
	}).prop('checked', true);
...
function simpan () {
	...
	if (isValid) {
		...
		$.ajax({
			...
			data: "table=instansi&act="+act+"&"+$('#form_input :input').serialize(),
			dataType: 'json',
			beforeSend : function (){
				$.messager.progress();
			},
...
function hapus () {
	...
	if (row){
		$.messager.confirm('Confirm','Anda Yakin Menghapus Data Ini ?',function(r){
			if (r){
				$.post('data/process/proses_master.php',{act:'delete', table:'instansi', id:row.KODEINSTANSI},function(msg){
					if (msg.success){
						$('#table_data').datagrid('reload');    // reload the user data
					} else {
						$.messager.alert('Error', msg.errorMsg, 'error');
...
function buat_table () {
	$('#table_data').datagrid({
		...
		pagination:true,
		pageSize:20,
		url: 'config/datagrid.php?table=instansi',
		rowStyler: function(index,row){
			if (row.STATUS == 0) return 'background-color:#a8aea6';
		},
		frozenColumns:[[
			{field:'KODEINSTANSI',title:'Kode',width:130, sortable:true,},
			{field:'NAMAINSTANSI',title:'Nama',width:200, sortable:true,},
		]],
		columns:[[
